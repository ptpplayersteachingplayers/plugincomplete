...
class PTP_Camp_Checkout_V99 {
    ...
    public function save_order_meta($order, $data) {
        ...
        error_log('[PTP Checkout v99] save_order_meta called for order #' . $order->get_id());
        ...
        if ($camper_first) {
            ...
            error_log('[PTP Checkout v99] Saved camper: ' . $full_name);
        }
        
        if (!empty($campers_data)) {
            $order->update_meta_data('_ptp_campers_data', $campers_data);
            error_log('[PTP Checkout v99] Saved ' . count($campers_data) . ' campers to _ptp_campers_data');
            ...
            foreach ($order->get_items() as $item_id => $item) {
                ...
                if ($is_camp) {
                    ...
                    if (!empty($item_campers)) {
                        ...
                        error_log("[PTP Checkout v99] Assigned " . count($item_campers) . " campers to item #$item_id");
                    }
                }
            }
        }
        ...
        error_log('[PTP Checkout v99] Order meta saved successfully');
    }
    ...
    public function on_order_created($order_id) {
        ...
        error_log('[PTP Checkout v99] on_order_created: Setting tracking cookie for order #' . $order_id);
    ...
    public function render_discounts_section($checkout) {
        ...
        <div class="ptp-section">
            ...
            if ($camps_in_cart > 1) :
                ...
                $discount_pct = $camps_in_cart >= 3 ? 20 : 10;
            ?>
