...
class PTP_Camp_Checkout_V98 {
    ...
    private function is_camp_checkout() {
        ...
        foreach ($cart_items as $item) {
            ...
            if ($product_name && (stripos($product_name, 'camp') !== false || stripos($product_name, 'clinic') !== false)) return true;
        }
        return false;
    }
    ...
    private function sess($key, $default = null) {
        ...
        if (class_exists('WooCommerce') && function_exists('WC') && WC()->session) {
            return WC()->session->get("ptp98_{$key}", $default);
        } elseif (function_exists('ptp_session')) {
            return ptp_session()->get("ptp98_{$key}", $default);
        }
        return $default;
    }
    ...
    private function set_sess($key, $val) {
        // v149: Support both WC and native session
        if (class_exists('WooCommerce') && function_exists('WC') && WC()->session) {
            WC()->session->set("ptp98_{$key}", $val);
        } elseif (function_exists('ptp_session')) {
            ptp_session()->set("ptp98_{$key}", $val);
        }
    }
